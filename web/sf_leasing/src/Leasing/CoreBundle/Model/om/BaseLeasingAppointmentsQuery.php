 ...
 * @method LeasingAppointmentsQuery orderByLeasePeriod($order = Criteria::ASC) Order by the lease_period column
 * @method LeasingAppointmentsQuery orderByTargetMoveIn($order = Criteria::ASC) Order by the target_move_in column
 ...
 * @method LeasingAppointmentsQuery groupByLeasePeriod() Group by the lease_period column
 * @method LeasingAppointmentsQuery groupByTargetMoveIn() Group by the target_move_in column
 ...
 * @method LeasingAppointments findOneByLeasePeriod(int $lease_period) Return the first LeasingAppointments filtered by the lease_period column
 * @method LeasingAppointments findOneByTargetMoveIn(string $target_move_in) Return the first LeasingAppointments filtered by the target_move_in column
 ...
 * @method array findByLeasePeriod(int $lease_period) Return LeasingAppointments objects filtered by the lease_period column
 * @method array findByTargetMoveIn(string $target_move_in) Return LeasingAppointments objects filtered by the target_move_in column
...
abstract class BaseLeasingAppointmentsQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `appointment_leads_id`, `unit_id`, `preferred_date`, `preferred_time`, `lease_period`, `target_move_in`, `confirmation_code`, `start_date`, `end_date`, `first_heard`, `notes`, `date_added`, `status`, `prev_status`, `processing`, `processed_by` FROM `appointments` WHERE `id` = :p0';
        try {
    ...
    /**
     * Filter the query on the lease_period column
     *
     * Example usage:
     * <code>
     * $query->filterByLeasePeriod(1234); // WHERE lease_period = 1234
     * $query->filterByLeasePeriod(array(12, 34)); // WHERE lease_period IN (12, 34)
     * $query->filterByLeasePeriod(array('min' => 12)); // WHERE lease_period >= 12
     * $query->filterByLeasePeriod(array('max' => 12)); // WHERE lease_period <= 12
     * </code>
     *
     * @param     mixed $leasePeriod The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return LeasingAppointmentsQuery The current query, for fluid interface
     */
    public function filterByLeasePeriod($leasePeriod = null, $comparison = null)
    {
        if (is_array($leasePeriod)) {
            $useMinMax = false;
            if (isset($leasePeriod['min'])) {
                $this->addUsingAlias(LeasingAppointmentsPeer::LEASE_PERIOD, $leasePeriod['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($leasePeriod['max'])) {
                $this->addUsingAlias(LeasingAppointmentsPeer::LEASE_PERIOD, $leasePeriod['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(LeasingAppointmentsPeer::LEASE_PERIOD, $leasePeriod, $comparison);
    }

    /**
     * Filter the query on the target_move_in column
     *
     * Example usage:
     * <code>
     * $query->filterByTargetMoveIn('fooValue');   // WHERE target_move_in = 'fooValue'
     * $query->filterByTargetMoveIn('%fooValue%'); // WHERE target_move_in LIKE '%fooValue%'
     * </code>
     *
     * @param     string $targetMoveIn The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return LeasingAppointmentsQuery The current query, for fluid interface
     */
    public function filterByTargetMoveIn($targetMoveIn = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($targetMoveIn)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $targetMoveIn)) {
                $targetMoveIn = str_replace('*', '%', $targetMoveIn);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(LeasingAppointmentsPeer::TARGET_MOVE_IN, $targetMoveIn, $comparison);
    }
