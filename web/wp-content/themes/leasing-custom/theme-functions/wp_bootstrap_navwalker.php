    ...
    {    
        ...
        function start_lvl(&$output, $depth)
        {
            ...
            if ($depth == 0) { //really, level-1 or level-2, because $depth is misleading here (see note above)
                $output .= "\n{$tabs}<ul class=\"dropdown-menu\" id=\"levelOne\">\n";
            } elseif ($depth == 1) {
                $output .= "\n{$tabs}<ul class=\"dropdown-menu\" id=\"levelTwo\">\n";
            } else {
                $output .= "\n{$tabs}<ul>\n";
            }
            return;
        ...
        {    
            ...
            if ($item->hasChildren) {
                // level-1 menus need the 'dropdown-submenu' class
                if($depth == 1) {
            ...
            switch ($item->title) {
                case 'Home':
                    ...
                    break;
                case 'Residential Leasing':
                    $icon = '<i class="icon-key-1"></i>';
                    break;
                case 'Commercial Leasing':
                    $icon = '<i class="icon-building"></i>';
                    break;
                case 'Events Place':
                    $icon = '<i class="icon-ticket"></i>';
                    break;
                case 'Sports Venue':
                    $icon = '<i class="icon-swimming"></i>';
                    break;
